...
typedef void(*send_data_cb_t)(char*,uint32_t);
typedef void(*recv_data_cb_t)(char*,uint32_t);
...
//histroy
    ...
    uint8_t hist_count;
    uint8_t exit;
}shell_context_struct_t,*p_shell_context_struct_t;

typedef int32_t(*cmd_function_t)(p_shell_context ,uint32_t ,char** argv);
